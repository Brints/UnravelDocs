...
public class OcrDocumentController {
    ...
    @Operation(
            summary = "Extract text from a specific file",
            description = "Triggers the OCR process to extract text from a file. If already processed, returns existing data.",
            responses = {
                    @ApiResponse(responseCode = "200", description = "Successfully extracted text or retrieved existing data",
                            content = @Content(mediaType = "application/json",
                                    schema = @Schema(implementation = OcrData.class))),
    ...
    @Operation(
            summary = "Extract text from all files in a collection",
            description = "Triggers the OCR process to extract text from all files in a collection.",
            responses = {
                    @ApiResponse(responseCode = "200", description = "Successfully extracted text from all files",
                            content = @Content(mediaType = "application/json",
                                    schema = @Schema(implementation = OcrData.class))),
                    @ApiResponse(responseCode = "403", description = "Forbidden - User not authorized or not logged in"),
    ...
    public ResponseEntity<DocumentCollectionResponse<DocumentCollectionUploadData>> extractTextFromAllFiles(
        ...
        DocumentCollectionResponse<DocumentCollectionUploadData> response = ocrService.uploadDocuments(files, user);

        return ResponseEntity.ok(response);
    }

    @Operation(
            summary = "Get OCR results for a document collection",
            description = "Retrieves the OCR results for a specific document collection.",
            responses = {
                    @ApiResponse(responseCode = "200", description = "Successfully retrieved OCR results",
                            content = @Content(mediaType = "application/json",
                                    schema = @Schema(implementation = CollectionResultResponse.class))),
                    @ApiResponse(responseCode = "403", description = "Forbidden - User not authorized or not logged in"),
                    @ApiResponse(responseCode = "404", description = "Not Found - Collection not found"),
    ...
    public ResponseEntity<DocumentCollectionResponse<CollectionResultResponse>> getOcrResults(
        ...
        DocumentCollectionResponse<CollectionResultResponse> response = ocrService.getCollectionResult(collectionId);

        return ResponseEntity.ok(response);
    }

    @Operation(
            summary = "Get OCR data for a specific document",
            description = "Retrieves the OCR data for a specific document in a collection.",
            responses = {
                    @ApiResponse(responseCode = "200", description = "Successfully retrieved OCR data",
                            content = @Content(mediaType = "application/json",
                                    schema = @Schema(implementation = OcrData.class))),
                    @ApiResponse(responseCode = "403", description = "Forbidden - User not authorized or not logged in"),
    ...
    public ResponseEntity<DocumentCollectionResponse<FileResultData>> getOcrData(
        ...
        User user = getAuthenticatedUser(authenticatedUser);
        DocumentCollectionResponse<FileResultData> response =
                ocrService.getOcrData(collectionId, documentId, user.getId());
